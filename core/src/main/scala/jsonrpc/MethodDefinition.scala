...
object MethodDefinition {
  implicit def EmtpyResponseFormat[A](implicit ev: A =:= Unit): Format[A] =
    Format[Unit](Reads.pure(()), Writes.apply(_ => Json.obj())).asInstanceOf[Format[A]]

  def create[A: Format, B: Format](methodName: String) =
    new MethodDefinition[A, B](methodName, implicitly[Format[A]], implicitly[Format[B]])
}
